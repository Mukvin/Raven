...
def run():
    ...
    engine_module = __import__(f'engines/{engine_name}')
    global engine
    ...
    for event in timeline['events']:
        t = Timer(event['at_hour'] * 3600, timed_exec, [event])
        t.start()
...
def timed_exec(e):
    ...
    subprocess.Popen(f'engines/{engine_name}/{e["name"]}.py')

    if e['name'] == 'ON_ELT_FINISH' and not no_load_incr_partitions:
        load_inr_partitions()
    ...
    if e['name'] in ('ON_PM_QUERY_START', 'ON_AM_QUERY_START'):
        ...
        todo_num = len(todos)
        logging.info(f"{todo_num} {am_or_pm} queries start submitting ...")
        futures = []
        result_book = [gl.AM_times if e['name'] == 'ON_AM_QUERY_START' else gl.PM_times]
        while len(todos) > 0:
            sql_and_time = todos.popleft()
            diff = time.time() - query_time_zero - sql_and_time['at_second']
            if diff > 0:
                time.sleep(diff)
            logging.info(
        ...
        logging.info(f"{todo_num} queries are finished")
...
def init_glue_tables():
    ...
    r = cf_r.create_stack(
        StackName=f'raven-stack-{random_identity}',
    ...
    for db_dict in historical_partitions['databases']:
        for table_dict in db_dict['tables']:
            ...
            for partition_dict in table_dict['partitions']:
                temp = partition_input_template.copy()
                values = []
                for key in partition_dict:
                    values += partition_dict[key]
                temp['Values'] = values
                location = table_dict['partition_location_template']
                for v in values:
                    location = str(location).replace('%value%', v, 1)
